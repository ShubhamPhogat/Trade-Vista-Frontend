  ...
  const drawChart = () => {
    ...
    if (visibleData.length > 0) {
      ...
      visibleData.forEach((data, index) => {
        ...
        ctx.fillRect(x, height - volHeight, bodyWidth, volHeight);
      });
  ...
  return (
    <div className={`trading-chart-container ${className || ""}`}>
      <div className="chart-wrapper w-full h-full bg-slate-900 rounded-lg p-4">
        {loading && (
          <div className="loading text-white">Loading chart data...</div>
        )}
        {error && <div className="error text-red-500">{error}</div>}
        {chartData.length === 0 && !loading && !error && (
          <div className="text-white">
            No data available. Check console for response details.
          </div>
        )}
